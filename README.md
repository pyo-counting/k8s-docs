...
- k8s는 구성 요소간의 안전한 통신, 인증을 위해 PKI x.509 server certificate 인증서를 사용한다. 기본적으로 k8s general 목적, etcd, front-proxy(옵션)를 위한 ica가 필요하다. 이후 필요한 인증서는 k8s에 위임할 수 있다(private key를 cluster에 복사하는 작업이 위험하다고 생각하면 필요한 인증서를 모두 직접 생성할 수도 있다). 뿐만 아니라 sa에서 사용하는 public, private key 쌍도 필요하다. ([PKI certificates and requirements](https://kubernetes.io/docs/setup/best-practices/certificates/#configure-certificates-manually))
