...
- managed node group의 특성은 다음과 같다. ([Managed node groups](https://docs.aws.amazon.com/eks/latest/userguide/managed-node-groups.html))
  ...
  - launch template은 인스턴스 레벨의 공통 security group 또는 eni 레벨의 security group을 지정할 수 있다. 만약 launch template에 security group을 설정할 경우 eks는 cluster security group을 추가하지 않기 때문에 eks control plane과의 정상적인 통신을 위해 security group에 관련 규칙도 추가해야 한다. ([Launch templates](https://docs.aws.amazon.com/eks/latest/userguide/launch-templates.html#launch-template-security-groups))
  - managed node group에 사용할 launch template에 user data는 ami(amazon linux ami의 경우 MIME multi-part archive, bottlerocket ami의 경우 toml)에 맞는 형식으로 작성해야 한다. 왜냐하면 eks가 no가 cluster에 join하는 데 필요한 eks user data와 자동으로 병합되기 때문이다. 추가적으로 사용하는 kubelet을 시작하거나 수정하는 명령어를 사용하면 안된다(예를 들어 eks optimized ami의 bootstrap.sh). kubelet 관련 작업은 eks user data에서 자동으로 수행하기 때문이다. launch template에 ami를 설정하는 경우에는 eks가 user data를 병합하지 않기 때문에 no가 cluster에 join할 수 있도록 user data에 kubelet 실행 관련 명령어를 지정해야 한다. 이 경우 사용자는 kubelet 명령어에 여러 flag를 설정할 수 있다. ([Launch templates](https://docs.aws.amazon.com/eks/latest/userguide/launch-templates.html#launch-template-user-data))
