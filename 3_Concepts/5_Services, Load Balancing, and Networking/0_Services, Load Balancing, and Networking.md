...
- svc를 사용하면 하나 이상의 백엔드 po에 의해 구현되는 서비스에 대해 안정적인 IP 주소나 hostname을 제공할 수 있다. 이때 svc를 구성하는 각각의 po는 시간이 지남에 따라 변경될 수 있다.
    - k8s는 현재 svc를 지원하는 po에 대한 정보를 제공하기 위해 endpointslice를 자동으로 관리한다.
    - service proxy 구현체는 svc, endpointslice를 모니터링하고 OS나 cloud provider API를 사용해 패킷을 가로채거나 재작성함으로써 svc 트래픽을 백엔드로 라우팅하는 data plane을 구성한다.
- gtw(또는 이전 버전인 ing)를 사용해 cluster 외부의 클라이언트가 svc에 접근할 수 있도록 할 수 있다.
    - 일부 cloud provider의 경우 svc의 `type: LoadBalancer`를 통해 더 간단하지만 설정 기능이 적은 ing를 사용할 수 있다.
- netpol는 po 간 또는 po와 외부 세계 간의 트래픽을 제어할 수 있게 한다.
...
- k8s는 kube-proxy라는 기본 service proxy 구현체를 제공하지만 일부 po network 구현체는 나머지 구현과 더 긴밀하게 통합된 자체 service proxy를 대신 사용한다.
- netpol 역시 일반적으로 po network 구현체에 의해 구현된다. (일부 po network 구현체는 netpol를 구현하지 않거나, 관리자가 netpol 지원 없이 po network를 구성하도록 선택할 수 있다. 이런 경우 API는 존재하지만 아무런 효과가 없다)
- [gtw 구현체](https://gateway-api.sigs.k8s.io/implementations/)들이 많이 있으며 일부는 특정 cloud 환경에 특화되어 있고 일부는 bare metal 환경에 더 중점을 둔다.
