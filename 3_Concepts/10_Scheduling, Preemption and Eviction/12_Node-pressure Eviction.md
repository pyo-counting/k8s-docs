node-pressure eviction은 kubelet이 no의 제한적인 리소스를 회수하기 위해 사전에 po를 종료하는 프로세스다.
...
kubelet은 cluster nod의 메모리, 디스크 공간, 파일 시스템 inode와 같은 리소스를 모니터링한다. 이러한 리소스 중 하나 이상이 특정 소비 레벨에 도달하면 kubelet은 해당 no에서 하나 이상의 po를 사전에 실패시켜 리소스를 회수하고 기아(starvation)를 방지할 수 있다.
...
kubelet은 사용자의 po를 종료하기 전에 [reclaim node-level resources](containerfs)를 시도한다. 예를 들어 디스크 리소스가 부족할 때 먼저 사용하지 않는 container image를 삭제한다.
